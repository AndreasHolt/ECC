...
const userPublicKeyHTML = document.getElementById('userPublicKey');
const userPrivateKeyHTML = document.getElementById('userPrivateKey');
...
*/
const humanUser = new User('Human', curve);
...
document.getElementById('inputMessageForm').addEventListener('submit', (Event) => {
    ...
    for (const user of users) {
        const textOut = document.getElementById(`encryptedText${user.label}`);
        const encryptedResult = encrypt(curve, inputField.value, humanUser, user);
